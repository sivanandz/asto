...
class VedicChartWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: CustomPaint(
        size: Size(size, size),
        painter: VedicChartPainter(chart: chart, style: style),
      ),
    );
  }
}
...
class VedicChartPainter extends CustomPainter {
  ...
  VedicChartPainter({required this.chart, required this.style});
  ...
  void _drawNorthIndianHouses(
    ...
    for (int row = 0; row < 3; row++) {
      for (int col = 0; col < 3; col++) {
        int? houseNum;
        if (row == 0)
          houseNum = housePositions[0][col];
        else if (row == 1 && col == 0)
          houseNum = housePositions[1][0];
        else if (row == 1 && col == 2)
          houseNum = housePositions[1][2];
        else if (row == 2)
          houseNum = housePositions[2][col];
        ...
        final planets = chart.positions
            .where((p) => p.house == actualHouse)
            .toList();
