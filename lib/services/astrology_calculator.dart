...
class AstrologyCalculator {
  ...
  static double _calculateSunLongitude(double jd) {
    ...
    // Mean anomaly
    final g = radians((357.528 + 0.9856003 * d) % 360);
    // Ecliptic longitude
  ...
  static double _calculateMoonLongitude(double jd) {
    ...
    // Mean anomaly
    final m = radians((134.963 + 13.064993 * d) % 360);
  ...
  static double _calculateAscendant(
    ...
    // Simplified formula
    var asc = degrees(math.atan2(
      -math.cos(radians(lst)),
      math.tan(radians(latitude)) * math.sin(radians(obliquity)) -
      math.sin(radians(lst)) * math.cos(radians(obliquity)),
    ));
