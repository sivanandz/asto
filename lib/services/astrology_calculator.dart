...
class AstrologyCalculator {
  ...
  @visibleForTesting
  static double calculateJulianDay(int year, int month, int day, double hour) {
    if (month <= 2) {
      year -= 1;
      month += 12;
    }
  ...
  static double _calculateSunLongitude(double jd) {
    ...
    // Mean anomaly
    final g = vector_math.radians((357.528 + 0.9856003 * d) % 360);
    // Ecliptic longitude
  ...
  static double _calculateMoonLongitude(double jd) {
    ...
    // Mean anomaly
    final m = vector_math.radians((134.963 + 13.064993 * d) % 360);
  ...
  static double _calculateAscendant(
    ...
    // Simplified formula
    var asc = vector_math.degrees(math.atan2(
      -math.cos(vector_math.radians(lst)),
      math.tan(vector_math.radians(latitude)) * math.sin(vector_math.radians(obliquity)) -
      math.sin(vector_math.radians(lst)) * math.cos(vector_math.radians(obliquity)),
    ));
